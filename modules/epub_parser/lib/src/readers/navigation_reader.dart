...
import 'package:epub_parser/src/schema/opf/epub_version.dart';
...
class NavigationReader {
  ...
  static Future<EpubNavigation> readNavigation(Archive epubArchive,
    ...
    EpubNavigationHead? navigationHead;
    late EpubNavigationMap navigationMap;
    late EpubNavigationDocTitle navigationDocTitle;
    final docAuthors = <EpubNavigationDocAuthor>[];
    EpubNavigationPageList? navigationPageList;
    final navigationList = <EpubNavigationList>[];
    ...
    if (package.Version == EpubVersion.Epub2) {
      ...
      navigationHead = readNavigationHead(headNode);
      ...
      navigationDocTitle = readNavigationDocTitle(docTitleNode);
      ...
      navigationMap = readNavigationMap(navMapNode);
      ...
      if (pageListNode != null) {
        navigationPageList = readNavigationPageList(pageListNode);
      }

      ncxNode
          .findElements('navList', namespace: ncxNamespace)
          .forEach((xml.XmlElement navigationListNode) {
        var navigationListItem = readNavigationList(navigationListNode);
        navigationList.add(navigationListItem);
      });
    } else {
      //Version 3
      ...
      navigationDocTitle = EpubNavigationDocTitle();
      navigationDocTitle.Titles = package.Metadata!.Titles;
//      result.DocTitle.Titles.add(headNode.findAllElements("title").firstWhere((element) =>  element != null, orElse: () => null).text.trim());
      ...
      navigationMap = readNavigationMapV3(navMapNode);
